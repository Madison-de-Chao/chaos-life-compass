...
const ApiDocsPage = () => {
  ...
  const handleDownloadDocs = () => {
    ...
    link.href = url;
    link.download = 'entitlements-api-docs.md';
...
\`\`\`bash
curl -X GET "${supabaseUrl}/functions/v1/check-entitlement?product_id=report_platform&email=user@example.com" \\
...
const checkEntitlement = async (email, productId) => {
  const response = await fetch(
    \`${supabaseUrl}/functions/v1/check-entitlement?product_id=\${productId}&email=\${email}\`,
    {
      headers: {
...
\`\`\`bash
curl -X GET "${supabaseUrl}/functions/v1/check-entitlement?product_id=story_builder_hub&email=user@example.com" \\
  -H "X-API-Key: your-api-key"
\`\`\`
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="default" size="sm" onClick={handleDownloadDocs}>
